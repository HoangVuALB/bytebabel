...
class SonioxTranscriber:
    ...
    def _run(self, audio_stream: Iterator[bytes]) -> None:
        ...
        producer = threading.Thread(target=_audio_producer, daemon=True, name="audio-producer")
        producer.start()
        ...
        try:
            ...
            while not self._stop_event.is_set():
                log.info(
                    ...
                    self._language, self._enable_diarization,
                )
                try:
            ...
            log.info(
                ...
                len(self._final_tokens), len(self._trans_final_tokens),
            )
            self._on_update(
                TranscriptUpdate(
    ...
    def _run_session(
        ...
        with client.realtime.stt.connect(config=config) as session:
                ...
                try:
                    while not self._stop_event.is_set():
                        ...
                        try:
                            ...
                            log.warning("Sender stopped: %s (sent %d chunks)", exc, chunks_sent)
                            break
            ...
            sender = threading.Thread(target=_send_audio, daemon=True, name="soniox-sender")
            sender.start()

            for event in session.receive_events():
                if event.error_code:
                    raise RuntimeError(f"Soniox error {event.error_code}: {event.error_message}")
                ...
                self._on_update(
                    TranscriptUpdate(
                        ...
                        translated_final_text=_join_tokens(self._trans_final_tokens),
                        translated_non_final_text=_join_tokens(self._trans_non_final_tokens),
                    )
                )
                self._non_final_tokens.clear()
                self._trans_non_final_tokens.clear()
